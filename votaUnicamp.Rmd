...
LIIC = as.numeric(as.character(dadosporInstituto[,5])) - qnorm(.975) * (sqrt((1 - (as.numeric(as.character(dadosporInstituto[,3])) / as.numeric(as.character(dadosporInstituto[,6])))) / (4*(as.numeric(as.character(dadosporInstituto[,3])))-1)))

ICinf = ifelse(LIIC < 0 | LIIC=="NaN" , 0 , LIIC)

LSIC = as.numeric(as.character(dadosporInstituto[,5])) + qnorm(.975) * (sqrt((1 - (as.numeric(as.character(dadosporInstituto[,3])) / as.numeric(as.character(dadosporInstituto[,6])))) / (4*(as.numeric(as.character(dadosporInstituto[,3])))-1)))

ICsup=ifelse(LSIC > 1 | LSIC=="NaN" , 1 , LSIC)
...
tamanhoIC = 0.1
regulares=as.numeric(as.character(dadosporInstituto2[,6]))
regulares2=ifelse(regulares=="NaN",0,regulares)

N = regulares2/(4*(qnorm(0.975)/tamanhoIC)^2*(regulares2-1)+1)
...
Aplicando a técnica de bootstrap obteve-se que `r round((resposta$t0),3)` % dos alunos são contrários a greve.
