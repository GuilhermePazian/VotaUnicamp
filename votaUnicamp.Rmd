---
<<<<<<< HEAD
title: "Projeto ME524 : Vota Unicamp!"
author: 
...
abstract: "A proposta de trabalho é de realizar análises estatísticas sobre os resultados obtidos pela 
...
<<<<<<< HEAD
Logo, como alternativa ao que consideram a não representatividade dos alunos por 
parte dos orgãos responsáveis, a pesquisa através do site  foi criada.
...
<<<<<<< HEAD
names(prop_tbl)=c("Total","Sim","Abstencao","Nao")
=======
>>>>>>> origin/master
...
#Leitura de arquivos relacionados a informações sobre os cursos de graduação 

setwd(choose.dir()) #escolha o diretório onde os arquivos do github foram 
#clonados
fvotosg = read.csv2("Matriculados_em_Cursos_Regulares_Graduação_Por_Curso_e_Turno_2016.csv") #esse arquivo contém a quantidade de alunos 
#regulares da graduação disponível em 
#<http://www.siarh.unicamp.br/indicadores/View.jsf?categoria=PUB_ACAD_MATRICULADO
#S>
...
#Leitura de arquivos relacionados a informações sobre os cursos de pós-graduação 

fvotosp = read.csv2("Matriculados_em_Cursos_Regulares_Pós_Graduação_Stricto_Sensu
                    _Por_Curso_e_Turno_2016.csv") #esse arquivo contém a quantidade de alunos regulares da Pós graduação disponível em <http://www.siarh.unicamp.br/indicadores/View.jsf?categoria=PUB_ACAD_MATRICULADOS>
...
``` {r barras,echo=FALSE}
barplot(c(sum(mytbl$sim),sum(mytbl$nao),sum(mytbl$abstencao)),
        legend=colnames(mytbl[5:7])
        ,col=c("gold1","darkorchid2","antiquewhite2")
        ,ylab = "Frequência"
        , main = "Total de Votos")

```

Tabela geral com os dados obtidos

```{r tabela_geral}
suppressPackageStartupMessages(library(DT))
datatable(data=mytbl)
```
...
library(d3heatmap)
library(RColorBrewer)
d3heatmap(mytbl[5:7],scale=c("Sim","nao","abstencao"),col=rev(brewer.pal(12,"Paired")),yaxis_width = 200)
```
...
### Estatísticas por área de estudo
...
esp=rbind(rep(1/3,3),rep(1/3,3),rep(1/3,3),rep(1/3,3),rep(1/3,3))

obs=rbind(prop_artes,prop_biologicas,prop_exatas,prop_humanas,prop_tecnologicas)

teste=chisq.test(esp,obs)
...
qtd = read.csv2(file.choose())

tabela.samp <- read.csv2(file.choose())
