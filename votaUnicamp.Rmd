...
pacotes = c("shiny","DT","tm","SnowballC","wordcloud","RColorBrewer","RCurl","XML","stringr","rvest","googleVis","leaflet","pracma","dplyr","d3heatmap","RColorBrewer", "ggrepel", "ggplot2", "ggmap")
...
```{r input, message=FALSE, echo = FALSE,message=FALSE}
...
```{r teste_qui_prop,echo=FALSE,message=FALSE}
...
Temos que o p-valor é aproximadamente de: `r p_valor` 

## Estatísticas por Instituto

```{r Institutos, echo = FALSE}
...
data.InstitutosRei = data.Instituto[-25,] #retirada da reitoria (profis != graduação)
alunosMatricPorInst = data.frame(read.csv2('QtdAlunosporInstituto.csv'))
regularesGrad = rep(0,length(data.InstitutosRei$V1))
for(i in 1:length(data.InstitutosRei$V1)){
...
LIIC = as.numeric(dadosporInstituto[,5])- qnorm(.975) * (sqrt(4*as.numeric(dadosporInstituto[,3])))^(-1)
LSIC = as.numeric(dadosporInstituto[,5]) + qnorm(.975) * (sqrt(4*as.numeric(dadosporInstituto[,3])))^(-1)
dadosporInstituto2 = data.frame(cbind(dadosporInstituto,LIIC,LSIC))
```

```{r GraficoDeDispersão,echo = FALSE}
#não sei porque o grafico não está sendo plotado, alguem manja? 
library(ggplot2)
p1 = as.numeric(dadosporInstituto2$naoInstituto)/as.numeric(dadosporInstituto2$totalInstituto)
p2 = as.numeric(dadosporInstituto2$totalInstituto)/as.numeric(dadosporInstituto2$regularesGrad)
siglas = as.character(dadosporInstituto2$V1)
ggplot(dadosporInstituto2, aes(x=p2, y=p1)) +
  geom_point() +
...
library(ggplot2)
graf<- ggplot(dadosporInstituto2, aes(x = as.numeric(dadosporInstituto2[,1]), y = dadosporInstituto2[,5])) +
  geom_point(size = 1) +
  geom_errorbar(aes(ymax = dadosporInstituto2[,8], ymin = dadosporInstituto2[,7]))
graf
```

```{r NnecessarioIC,echo=FALSE, results="hide"}
tamanhoIC = 0.1
N = (1/4) *(2*qnorm(0.975)/tamanhoIC)^2
Nnecessario = ceiling(N)
```

```{r tabelaAmostraNecessaria,echo=FALSE}
tabelaNnecessario = data.frame(Instituto = dadosporInstituto2[,1],Total_Instituto = dadosporInstituto2[,3],Tamanho_de_Amostra_Necessario = Nnecessario,votosFaltantes = (Nnecessario - as.numeric(dadosporInstituto2[,3])))
```

```{r tabelaInstituto,echo=FALSE}
library(DT)
...
```{r enderecos, echo = FALSE, message = FALSE}
