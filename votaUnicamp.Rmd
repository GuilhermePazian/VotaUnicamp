...
LIIC = max(0,as.numeric(dadosporInstituto[,5])- qnorm(.975) * (sqrt(4*as.numeric(dadosporInstituto[,3])))^(-1))
LSIC = min(1,as.numeric(dadosporInstituto[,5]) + qnorm(.975) * (sqrt(4*as.numeric(dadosporInstituto[,3])))^(-1))
dadosporInstituto2 = data.frame(cbind(dadosporInstituto,round(LIIC,3),round(LSIC,3)))
```
...
library(ggplot2)
graf<- ggplot(dadosporInstituto2, aes(x = as.numeric(dadosporInstituto2[,1]), y = as.character(dadosporInstituto2[,5]))) +
  geom_point(size = 1) +
  geom_errorbar(aes(ymax = dadosporInstituto2[,8], ymin = dadosporInstituto2[,7])) + xlab("Proporção de Votos Contra a Greve") + ylab("Institutos")
graf
```
