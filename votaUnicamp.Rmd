...
```{r codigo_wordcloud, echo = FALSE, eval = FALSE, message=FALSE}
### Funções auxiliares
...
```{r tabela_geral, echo = FALSE, message = FALSE}
library(DT)
...
#colnames(dadosporInstituto) = c("Instituto","Não","total","abstenção","proporção de Nãos","Alunos Regulares")    #alguem pode ajudar aqui??? porque não funciona?
library(DT)
datatable(data=dadosporInstituto,class = 'cell-border stripe')
...
library(ggplot2)
p1 = as.numeric(dadosporInstituto$naoInstituto)/as.numeric(dadosporInstituto$totalInstituto)
p2 = as.numeric(dadosporInstituto$totalInstituto)/as.numeric(dadosporInstituto$regularesGrad)
siglas = as.character(dadosporInstituto$V1)
ggplot(dadosporInstituto, aes(x=p2, y=p1)) +
  geom_point() +
